...
import clickhouse_connect  # clickhouse_connect 라이브러리로 변경됨
...
def init_db():
    """앱 시작 시 ClickHouse 테이블 자동 생성 (안전장치)"""
    try:
        client = get_ch_client()
        # IF NOT EXISTS를 통해 이미 테이블이 있다면 새로 만들지 않고 기존 테이블을 기억/유지합니다.
        client.command('''
...
def index():
    ...
    try:
        client = get_ch_client()
        # clickhouse_connect를 이용한 쿼리 실행
        result = client.query('SELECT id, category, title, filename, file_url, uploaded_at FROM image_data ORDER BY uploaded_at DESC')
        images = [
...
def upload():
    ...
    if file and allowed_file(file.filename):
        ...
        # 4. ClickHouse에 메타데이터 저장 (clickhouse_connect용 insert 구문)
        try:
            client = get_ch_client()
            img_id = uuid.uuid4()
            now = datetime.now()
            ...
            client.insert(
                ...
                column_names=['id', 'category', 'title', 'filename', 'file_url', 'uploaded_at']
            )
...
def delete(img_id):
    ...
    try:
        # 1. ClickHouse에서 데이터 삭제 (clickhouse_connect command 사용)
        client = get_ch_client()
