...
# --- Ollama 설정 (사용하시는 gpt-oss:20b 모델 지정) ---
OLLAMA_API_URL = "http://localhost:11434/api/generate"
OLLAMA_MODEL = "gpt-oss:20b"  # 사용 중인 로컬 대형 모델명 반영
...
<div class="tab-content" id="mainServiceTabContent">
  ...
  <div class="tab-pane fade show active" id="explorerContent" role="tabpanel">
      ...
      <div class="tab-pane fade show active" id="blueprint" role="tabpanel">
        <div class="row row-cols-1 row-cols-md-3 g-4">
            {% for img in images if img.category == 'blueprint' %}
                <div class="col"><div class="card h-100 shadow-sm" style="cursor: pointer;" onclick="showImageDetail('{{ img.file_url }}', '{{ img.title|replace(\"'\", \"\\\\'\") }}', '{{ img.serial_code }}', '{{ img.category }}', '{{ img.description|replace(\"'\", \"\\\\'\") }}', '{{ img.uploaded_at }}')">
    ...
    <!-- 탐색기 전용 공용 상세 패널 (AI 챗봇 사이드바 레이아웃 구조와 일관되게 표시) -->
    <div class="card shadow-sm mb-4">
        <div class="card-header bg-secondary text-white fw-bold">🔍 선택 이미지 상세 메타데이터</div>
        <div class="card-body text-center" id="explorerDetailPanel">
            <span class="text-muted" id="explorerDetailPlaceholder">데이터 탐색기에서 위 이미지를 선택하면 고해상도 상세 내용이 노출됩니다.</span>
            <div id="explorerDetailContent" class="text-start d-none w-100">
                <div class="row align-items-center">
                    <div class="col-md-5">
                        <img id="explorerDetailImg" class="img-fluid rounded border shadow-sm" style="max-height: 300px; object-fit: cover; width: 100%;">
                    </div>
                    <div class="col-md-7">
                        <h4 class="fw-bold mb-1" id="explorerDetailTitle"></h4>
                        <p class="badge bg-dark mb-3 fs-6" id="explorerDetailSerial"></p>
                        <hr class="my-2">
                        <p class="mb-2"><strong>카테고리:</strong> <span id="explorerDetailCategory" class="badge"></span></p>
                        <p class="mb-2"><strong>업로드 일시:</strong> <span id="explorerDetailDate" class="text-secondary"></span></p>
                        <p class="mb-1"><strong>상세 설명 및 작업 일지 메모:</strong></p>
                        <div class="p-3 bg-light rounded text-muted border" id="explorerDetailDesc" style="min-height: 100px;"></div>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    function showImageDetail(url, title, serial, category, desc, date) {
        // AI 챗봇 사이드바 제어
        const sidePlaceholder = document.getElementById('detailPlaceholder');
        if (sidePlaceholder) {
            sidePlaceholder.style.display = 'none';
        }
        const sideContent = document.getElementById('detailContent');
        if (sideContent) {
            ...
            const sideCatBadge = document.getElementById('detailCategory');
            setCategoryBadge(sideCatBadge, category);
        }

        // 데이터 탐색기 공용 상세 패널 제어
        const expPlaceholder = document.getElementById('explorerDetailPlaceholder');
        if (expPlaceholder) {
            expPlaceholder.style.display = 'none';
        }
        const expContent = document.getElementById('explorerDetailContent');
        if (expContent) {
            expContent.classList.remove('d-none');
            document.getElementById('explorerDetailImg').src = url;
            document.getElementById('explorerDetailTitle').innerText = title;
            document.getElementById('explorerDetailSerial').innerText = serial;
            document.getElementById('explorerDetailDate').innerText = date;
            document.getElementById('explorerDetailDesc').innerText = desc || '등록된 상세 설명이 없습니다.';
            
            const expCatBadge = document.getElementById('explorerDetailCategory');
            setCategoryBadge(expCatBadge, category);
        }
    }

    function setCategoryBadge(badgeElement, category) {
        if (!badgeElement) return;
        badgeElement.className = 'badge';
        if (category === 'blueprint') {
            badgeElement.innerText = '도면';
            badgeElement.classList.add('bg-info');
        } else if (category === 'factory') {
            badgeElement.innerText = '공장 전경';
            badgeElement.classList.add('bg-warning', 'text-dark');
        } else {
            badgeElement.innerText = 'Vision 데이터';
    ...
    document.getElementById('chatForm').addEventListener('submit', async function(e) {
        ...
        const userText = inputField.value.trim ? inputField.value.trim() : inputField.value;
        if (!userText) return;
            ...
            if (data.success) {
                ...
                if (data.images && data.images.length > 0) {
                    ...
                    data.images.forEach(img => {
                        ...
                        const safeDesc = img.description.replace(/'/g, "\\'");
                        htmlContent += `
                            <img src="${img.file_url}" class="chat-img-card" 
                                 onclick="showImageDetail('${img.file_url}', '${safeTitle}', '${img.serial_code}', '${img.category}', '${safeDesc}', '${img.uploaded_at}')" 
...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <div>
        <button type="button" class="btn btn-danger" id="bulkDeleteBtn" disabled onclick="submitBulkDelete()">
            🗑️ 선택 이미지 일괄 삭제 (<span id="selectedCount">0</span>)
        </button>
    </div>
</div>
    ...
    <div class="col-lg-7">
        <div class="card shadow-sm">
            <div class="card-header bg-secondary text-white d-flex justify-content-between align-items-center">
                <span>등록된 이미지 데이터 관리 리스트</span>
                <span class="badge bg-light text-dark">총 {{ images|length }}건</span>
            </div>
            <div class="card-body p-0">
                <form id="bulkDeleteForm" action="{{ url_for('delete_multiple') }}" method="POST">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td class="text-center">
                                        <input type="checkbox" name="delete_ids" value="{{ img.id }}" data-filename="{{ img.filename }}" onchange="updateSelectedStatus()">
                                    </td>
                                    ...
                                    <td>
                                        <button type="button" class="btn btn-danger btn-sm" onclick="deleteSingle('{{ img.id }}', '{{ img.filename }}')">삭제</button>
                                    </td>
                                </tr>
                                {% else %}
                                <tr><td colspan="6" class="text-center py-5 text-muted">검색 조건과 일치하거나 등록된 이미지가 없습니다.</td></tr>
                                {% endfor %}
                            </tbody>
                        </table>
                    </div>
...
<form id="singleDeleteForm" action="" method="POST" style="display:none;">
    <input type="hidden" name="filename" id="singleDeleteFilename">
</form>
...
<script>
    ...
    // 단일 삭제 액션 처리
    function deleteSingle(id, filename) {
    ...
    // 일괄 삭제 데이터 합산 후 submit 처리
    function submitBulkDelete() {
        const checkboxes = document.getElementsByName('delete_ids');
        const selectedFilenames = [];
        checkboxes.forEach(cb => {
            if (cb.checked) {
                selectedFilenames.push(cb.getAttribute('data-filename'));
            }
        });

        if (confirm('선택된 ' + selectedFilenames.length + '개의 데이터를 일괄 영구 삭제하시겠습니까?')) {
            // 파일 시스템 삭제를 전달하기 위해 동적으로 파일명 정보도 히든 필드로 동반 전송
...
def delete_multiple():
    """여러 이미지 데이터를 일괄 수집하여 ClickHouse 데이터 및 물리 파일을 영구히 삭제합니다."""
    if not session.get('is_admin'):
        return redirect(url_for('login'))
    ...
    if not delete_ids:
        flash('일괄 삭제할 이미지가 선택되지 않았습니다.', 'warning')
        return redirect(url_for('admin'))
    ...
    try:
        ...
        # 1. ClickHouse 비동기 Mutation 일괄 일치 삭제
        # 고속 처리를 위해 다중 쿼리 대신 IN 조건문을 빌드합니다.
        id_list_str = ", ".join([f"'{img_id}'" for img_id in delete_ids])
        client.command(f"ALTER TABLE image_data DELETE WHERE id IN ({id_list_str})")
        
        # 2. 로컬 스토리지에서 물리 파일 일괄 삭제
        ...
        for filename in filenames:
            ...
            if os.path.exists(filepath):
                try:
                    ...
                    print(f"물리 파일 삭제 누락 ({filename}): {file_err}")
                    
        flash(f'총 {len(delete_ids)}건의 데이터와 {deleted_file_count}개의 이미지가 성공적으로 일괄 삭제되었습니다.', 'success')
    except Exception as e:
        flash(f'일괄 삭제 수행 도중 예외가 발생했습니다: {e}', 'danger')
...
def chat():
    ...
    system_prompt = (
        ...
        "Do not include any explanation, greeting, or markdown code block formatting like ```json or ```. "
        "For example, if the user says '러시아 공장 전경 보여줘', "
        "your response MUST be exactly: {\"category\": \"factory\", \"keywords\": [\"러시아\"]}. "
        "If they say 'NG 발생한 비전 데이터', "
        "your response MUST be exactly: {\"category\": \"vision\", \"keywords\": [\"NG\", \"불량\"]}"
    )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # [수정 사항] gpt-oss:20b와 같은 대형 오픈소스 모델의 예기치 않은 JSON 출력 형식을 보장하기 위한 정규식 파서 탑재
            # 중괄호 사이의 텍스트가 있을 경우 이를 정확하게 포착합니다.
            json_match = re.search(r'(\{.*?\})', raw_response, re.DOTALL)
            if json_match:
                json_str = json_match.group(1)
                extracted_filter = json.loads(json_str)
            else:
                # 정규식 패턴 실패 시 가벼운 보정 처리
                if "```" in raw_response:
        ...
        # Ollama가 꺼져있거나 예외 발생 시 최소한의 형태소 예측 보완 로직 작동
    ...
    # 20b 모델이 카테고리를 찾지 못했으나 키워드에 포함되어 있을 경우의 수동 상향 피드백 보정
    if category == 'all' or not category:
        if any(w in user_message for w in ["도면", "설계"]):
            category = "blueprint"
        elif any(w in user_message for w in ["전경", "공장", "배경"]):
            category = "factory"
        elif any(w in user_message for w in ["비전", "vision", "NG", "불량", "합격"]):
            category = "vision"
    ...
    try:
        ...
        for i, keyword in enumerate(keywords):
            ...
            if clean_keyword:
                ...
                params[param_name] = f"%{clean_keyword}%"
        ...
        chatbot_prompt = (
            ...
            "사용자의 질문에 부합하는 분석(예: '러시아 공장 전경 사진 1건이 데이터베이스에서 필터링되었습니다.')을 포함해 주세요. "
            "출력된 이미지들의 리스트를 가볍게 설명해 주며 끝마쳐 주세요. 검색 결과가 아예 없다면 관련된 내용을 더 상세히 등록해 보라고 조언해 주세요."
        )
