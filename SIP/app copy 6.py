...
def get_filtered_images(args):
    ...
    q = args.get('q', '').strip()
    if q:
        # ClickHouse의 ILIKE는 대소문자를 구분하지 않는 고성능 LIKE 패턴 매칭을 제공합니다.
...
def index():
    ...
    return render_template('index.html', images=images, params=params, action_url=url_for('index'))
